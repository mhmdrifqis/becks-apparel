...
<div class="h-screen overflow-hidden flex flex-col bg-slate-50" x-data="customizer()" x-cloak>
    <style>
        ...
        ::-webkit-scrollbar-thumb { background: #1a4b36; border-radius: 10px; }
        ::-webkit-scrollbar-thumb:hover { background: #06402B; }
        
        .custom-scrollbar::-webkit-scrollbar { width: 4px; }
        .custom-scrollbar::-webkit-scrollbar-track { background: rgba(255, 255, 255, 0.05); border-radius: 10px; }
        .custom-scrollbar::-webkit-scrollbar-thumb { background: #06402B; border-radius: 10px; }
        ...
        .cropper-container { background-color: #03120d !important; }
        .cropper-modal { background-color: #03120d !important; opacity: 0.8 !important; }
        .cropper-view-box { outline: 2px solid #06402B !important; }
    </style>
    ...
    <div id="app-container" class="flex flex-col md:flex-row h-full w-full overflow-hidden relative">
        ...
        <aside 
            ...
            class="relative w-full bg-[#04251a] border-t md:border-t-0 md:border-r border-brand-800/20 flex flex-col z-20 shadow-2xl order-2 md:order-2 transition-all duration-300 overflow-hidden shrink-0 mb-16 md:mb-0"
        >
                        ...
                        <div class="grid grid-cols-7 gap-1.5">
                            <template x-for="(color, index) in solidColors" :key="'solid-'+color">
                                <button 
                                    ...
                                    :class="partColors[activePart] === color && partColorMode[activePart] === 'solid' ? 'ring-2 ring-brand-400 ring-offset-2 ring-offset-[#04251a] scale-110' : 'opacity-90 hover:opacity-100 hover:scale-105'" 
                        ...
                        <div class="grid grid-cols-7 gap-1.5">
                            <template x-for="(gp, index) in gradientPresets" :key="gp.id">
                                <button 
                                    ...
                                    :class="partActiveGradientPreset[activePart] === gp.id ? 'ring-2 ring-brand-400 ring-offset-2 ring-offset-[#04251a] scale-110' : 'opacity-90 hover:opacity-100 hover:scale-105'"
                                    class="w-7 h-7 rounded-full border border-white/5 transition-all shadow-lg"
                ...
                <div x-show="activeMenu === 'pattern'" x-transition class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-7 gap-1.5">
                                <template x-for="(color, index) in solidColors" :key="'pat-'+color">
                                    <button x-show="showAllPatternColors || index < 14" @click="updatePatternColor(color)" :style="`background-color: ${color}`" :class="patternColor === color ? 'ring-2 ring-brand-400 ring-offset-2 ring-offset-[#04251a] scale-110' : 'opacity-90 hover:opacity-100 hover:scale-105'" class="w-7 h-7 rounded-full border border-white/5 shadow-lg transition-all"></button>
                                </template>
                                <button class="w-7 h-7 rounded-full border-2 border-dashed border-brand-800 hover:border-brand-500 bg-brand-950/40 flex items-center justify-center relative transition-all hover:scale-105">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-7 gap-1.5">
                                <template x-for="(gp, index) in gradientPresets" :key="'pat-grad-'+gp.id">
                                    <button 
                                        ...
                                        :class="patternActiveGradientPreset === gp.id && activeState.partPatternGradientEnabled[activePatternPart] ? 'ring-2 ring-brand-400 ring-offset-2 ring-offset-[#04251a] scale-110' : 'opacity-90 hover:opacity-100 hover:scale-105'"
                                        class="w-7 h-7 rounded-full border border-white/5 transition-all shadow-lg"
                                        :style="`background: ${gp.type === 'radial' ? 'radial-gradient(circle, ' + gp.color1 + ', ' + gp.color2 + ')' : 'linear-gradient(' + gp.angle + 'deg, ' + gp.color1 + ', ' + gp.color2 + ')'}`"
                                    ></button>
                                </template>
                            ...
                            <!-- Custom Pattern Gradient Controls -->
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-7 gap-1.5">
                            <template x-for="(color, index) in solidColors" :key="'txt-'+color">
                                <button x-show="showAllTextColors || index < 14" @click="updateTextProperty('activeColor', color)" :style="`background-color: ${color}`" :class="activeColor === color && !textGradientEnabled ? 'ring-2 ring-brand-400 ring-offset-2 ring-offset-[#04251a] scale-110' : 'opacity-90 hover:opacity-100 hover:scale-105'" class="w-7 h-7 rounded-full border border-white/5 shadow-lg transition-all"></button>
                            </template>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-7 gap-1.5">
                            <template x-for="(gp, index) in gradientPresets" :key="'txt-grad-'+gp.id">
                                <button 
                                    ...
                                    :class="textActiveGradientPreset === gp.id && textGradientEnabled ? 'ring-2 ring-brand-400 ring-offset-2 ring-offset-[#04251a] scale-110' : 'opacity-90 hover:opacity-100 hover:scale-105'"
                                    class="w-7 h-7 rounded-full border border-white/5 transition-all shadow-lg"
                                    :style="`background: ${gp.type === 'radial' ? 'radial-gradient(circle, ' + gp.color1 + ', ' + gp.color2 + ')' : 'linear-gradient(' + gp.angle + 'deg, ' + gp.color1 + ', ' + gp.color2 + ')'}`"
                                ></button>
                            </template>
        ...
        <main class="flex-1 bg-white relative flex flex-col items-center justify-center p-4 min-h-[400px] order-1 md:order-3 mb-16 md:mb-0">
            ...
            <div class="absolute top-6 left-6 z-40 flex items-center gap-2">
                <button @click="window.history.back()" class="bg-white/80 backdrop-blur-xl text-slate-700 p-3 rounded-2xl border border-slate-200 hover:bg-slate-100 transition-all shadow-xl group" title="Kembali">
            ...
            <div class="absolute bottom-20 md:bottom-8 right-8 z-40 flex flex-col gap-3 items-end">
                <!-- Save -->
                <button class="bg-brand-900 hover:bg-brand-800 text-white w-14 h-14 rounded-2xl shadow-2xl flex items-center justify-center transition-all hover:scale-110 active:scale-95 border border-white/10 group" title="Simpan Desain">
            ...
            <div class="relative bg-white rounded-[2rem] md:rounded-[3rem] p-4 md:p-12 border border-slate-200 shadow-2xl w-full max-w-[600px] aspect-square flex items-center justify-center">
                <!-- CANVAS -->
                ...
                >
                    ...
                    <button @click="flipObject('X')" class="p-2.5 text-brand-300 hover:text-white hover:bg-brand-800 rounded-xl transition-colors"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18"/><path d="m15 15 3-3-3-3"/><path d="m9 9-3 3 3 3"/></svg></button>
                    <button @click="deleteSelected()" class="p-2.5 text-rose-400 hover:text-white hover:bg-rose-600 rounded-xl transition-all"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg></button>
                </div>
            </div>
        </main>
    </div>

    <!-- MODAL COPY CONFIRMATION -->
