...
<div x-data="{ 
    ...
    <div class="max-w-7xl mx-auto px-4 py-8 md:py-16">
            ...
            <div class="flex-1 p-6 md:p-10 space-y-8">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2 mt-4 text-xs font-bold text-slate-400 uppercase tracking-widest">
                        ...
                        <span class="w-1 h-1 bg-slate-200 rounded-full"></span>
                        <span class="text-brand-900">{{ $package->order_items_sum_quantity ? number_format($package->order_items_sum_quantity, 0, ',', '.') : '0' }} Terjual</span>
                    </div>
                </div>
        ...
        <div class="mt-12 bg-white p-8 md:p-12 border border-slate-100 shadow-sm rounded-[2.5rem]">
            <div class="mb-10 pb-4 border-b border-slate-100 flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <h2 class="text-xs font-black text-slate-900 uppercase tracking-[0.3em]">Ulasan Pelanggan</h2>
                    @php 
                        $reviews = $package->reviews()->with('user')->where('is_visible', true)->latest()->get(); 
                        $avgRating = $reviews->count() > 0 ? round($reviews->avg('rating'), 1) : 0;
                    @endphp
                    @if($reviews->count() > 0)
                        <div class="px-3 py-1 bg-amber-50 text-amber-600 rounded-lg flex items-center gap-1.5">
                            <svg class="w-4 h-4 fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                            <span class="text-xs font-black">{{ $avgRating }} / 5.0</span>
                        </div>
                    @endif
                </div>
                <div class="h-px bg-slate-100 flex-1 hidden md:block"></div>
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $reviews->count() }} Ulasan</span>
            </div>
                        ...
                        <div class="p-6 bg-slate-50 border border-slate-100 rounded-2xl">
                            <div class="flex items-start justify-between mb-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 font-black uppercase text-sm">
                                        {{ substr($review->user->name, 0, 1) }}
                                    </div>
                                    <div>
                                        <p class="text-[11px] font-black text-slate-900 uppercase tracking-widest">{{ $review->user->name }}</p>
                                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{{ $review->created_at->format('d M Y') }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-1">
                                    @for($i = 1; $i <= 5; $i++)
                                        <svg class="w-3.5 h-3.5 {{ $i <= $review->rating ? 'text-amber-400' : 'text-slate-200' }} fill-current" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                                    @endfor
                                </div>
                            </div>
                            
                            @if($review->comment)
                                <p class="text-[11px] font-medium text-slate-600 leading-relaxed italic mb-4">"{{ $review->comment }}"</p>
                            @endif

                            @if(!empty($review->images))
                                <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                                    @foreach($review->images as $img)
                                        <a href="{{ Storage::disk('public')->url($img) }}" target="_blank" class="w-16 h-16 rounded-xl border border-slate-200 overflow-hidden flex-shrink-0 block hover:border-brand-900 transition-colors">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-[10px] text-slate-400 mt-1">Jadilah yang pertama memberikan ulasan untuk produk ini!</p>
                </div>
            @endif
        </div>
    </div>
