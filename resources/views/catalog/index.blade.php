    ...
    <div class="max-w-7xl mx-auto px-4 py-12" x-data="{ activeCategory: new URLSearchParams(window.location.search).get('category') || 'all' }">
        <div class="flex flex-col lg:flex-row gap-12">
            ...
            <aside class="w-full lg:w-64 space-y-8 flex-shrink-0">
                <div>
                    <h3 class="font-display text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-4">Kategori Produk</h3>
                    <div class="space-y-2">
                        <button @click="activeCategory = 'all'" 
                                :class="activeCategory === 'all' ? 'bg-brand-900 text-white shadow-lg shadow-brand-900/20' : 'text-slate-600 hover:bg-slate-100'" 
                                class="w-full text-left px-5 py-3.5 rounded-2xl font-display text-sm font-black uppercase tracking-wider transition-all flex items-center justify-between group">
                            Semua Produk
                ...
                <div class="p-6 bg-brand-50 rounded-[2rem] border border-brand-100">
                    <p class="font-display text-xs font-black text-brand-900 uppercase tracking-widest mb-2">Butuh Bantuan?</p>
                    ...
                    <a href="https://wa.me/6285183327132" target="_blank" class="inline-block font-display text-xs font-black text-brand-900 uppercase tracking-widest border-b-2 border-brand-900 pb-1">Hubungi WhatsApp</a>
                </div>
            </aside>
            ...
            <main class="flex-1">
                ...
                <div x-show="activeCategory === 'all' || activeCategory === '{{ $category }}'" 
                    ...
                    <div class="flex items-center gap-4 mb-8">
                        <h2 class="font-display text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-wider">{{ $categories[$category] ?? ucfirst($category) }}</h2>
                        <div class="h-px bg-slate-200 flex-1"></div>
                        <span class="font-display text-xs font-bold text-slate-500 uppercase tracking-wider">{{ count($items) }} Models</span>
                    </div>
