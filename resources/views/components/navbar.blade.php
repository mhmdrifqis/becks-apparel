<nav x-data="{ 
    ...
    :class="{ 
        /* State 1: Floating Pill saat di-scroll (Desktop & Mobile) */
        'top-4 w-[92%] max-w-6xl mx-auto rounded-full bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl shadow-2xl py-1 border-transparent': isScrolled,
        
        /* State 2: Full-width sticky (Bukan Hero Page & Belum di-scroll) */
        'top-0 w-full bg-white/90 dark:bg-zinc-950/90 backdrop-blur-xl shadow-xl py-4 rounded-none': !isScrolled && !isHeroPage,
        
        /* State 3: Transparent (Hero Page & Belum di-scroll) */
        'top-0 w-full bg-transparent py-8 rounded-none': isTransparent 
    }"
    class="fixed left-0 right-0 z-50 transition-all duration-500 ease-in-out">
    
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-12 sm:h-16 items-center">
            <div class="flex items-center">
                <a href="{{ url('/') }}" class="flex items-center gap-3 group text-decoration-none">
                    <x-application-logo class="h-8 sm:h-10 w-auto transition-transform duration-300 group-hover:scale-110" 
                        ::class="isTransparent ? 'text-white' : 'text-brand-900 dark:text-brand-400'" />
                    <span class="font-black text-base sm:text-2xl tracking-tighter transition-colors duration-300" 
            ...
            <div class="hidden sm:flex sm:items-center sm:gap-10">
                ...
                @auth
                    <div class="flex items-center gap-6" x-data="{ userOpen: false, notifOpen: false }">
                        <div class="flex items-center gap-2">
                            @if(!Auth::user()->hasAnyRole(['Admin', 'Tim Produksi', 'Management/Owner']))
                                <div class="relative">
                                    <button @click="notifOpen = !notifOpen; if(notifOpen && notifications.length === 0) fetchNotifications()" @click.away="notifOpen = false" class="relative p-2 transition-colors group" :class="isTransparent ? 'text-white/80 hover:text-white' : 'text-slate-500 hover:text-brand-900'">
                                    ...
                                    <div x-show="notifOpen" x-cloak 
                                         ...
                                         class="absolute right-0 mt-4 w-80 rounded-3xl bg-white dark:bg-zinc-900 shadow-2xl border border-gray-100 dark:border-zinc-800 py-3 z-50 overflow-hidden">
                                        <div class="px-5 py-3 border-b border-gray-100 dark:border-zinc-800 flex justify-between items-center">
                                            <p class="text-xs font-black uppercase tracking-widest text-brand-900 dark:text-brand-400">Notifikasi</p>
                                            <button @click="markAllAsRead()" x-show="notifCount > 0" class="text-[10px] text-brand-500 hover:text-brand-700 font-bold uppercase">Tandai Semua Dibaca</button>
                                        </div>
                                        <div class="max-h-80 overflow-y-auto">
                                            ...
                                            <template x-for="notif in notifications" :key="notif.id">
                                                <a :href="notif.data.url || '#'" @click="if(!notif.read_at) markAsRead(notif.id)" class="block px-5 py-3 hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors border-b border-gray-50 dark:border-zinc-800/50 last:border-0" :class="!notif.read_at ? 'bg-brand-50/50 dark:bg-brand-950/20' : ''">
                                                    <div class="flex items-start gap-3">
                                                        <div class="mt-1">
                                                            <div class="w-2 h-2 rounded-full bg-brand-500" x-show="!notif.read_at"></div>
                                                        </div>
                                                        <div>
                                                            <p class="text-xs font-black text-gray-900 dark:text-white" x-text="notif.data.title"></p>
                                                            <p class="text-[10px] text-gray-500 mt-0.5 leading-relaxed" x-text="notif.data.message"></p>
                                                            <p class="text-[9px] text-gray-400 mt-1 uppercase font-semibold" x-text="notif.created_at"></p>
                                                        </div>
                                                    </div>
                                                </a>
                                            </template>
                        ...
                        <div x-show="userOpen" 
                             ...
                             x-transition:leave-start="opacity-100 scale-100"
                             x-transition:leave-end="opacity-0 scale-95"
                             class="absolute right-0 mt-4 w-64 rounded-3xl bg-white dark:bg-zinc-900 shadow-2xl border border-gray-100 dark:border-zinc-800 py-3 z-50 overflow-hidden">
            ...
            <div class="flex sm:hidden items-center gap-1 sm:gap-2" x-data="{ notifOpenMobile: false }">
                
                @auth
                    @if(!Auth::user()->hasAnyRole(['Admin', 'Tim Produksi', 'Management/Owner']))
                        <div class="relative">
                            <button @click="notifOpenMobile = !notifOpenMobile; if(notifOpenMobile && notifications.length === 0) fetchNotifications()" @click.away="notifOpenMobile = false" class="relative p-2 text-slate-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>
                                <template x-if="notifCount > 0">
                                    <span x-text="notifCount" class="absolute top-1 right-1 flex items-center justify-center min-w-[16px] h-[16px] px-1 bg-red-500 text-white text-[9px] font-black rounded-full border-2 border-white dark:border-zinc-950"></span>
                                </template>
                            ...
                            <!-- Mobile Dropdown Notifikasi -->
                            <div x-show="notifOpenMobile" x-cloak 
                                 x-transition:enter="transition ease-out duration-100"
                                 x-transition:enter-start="opacity-0 scale-95"
                                 x-transition:enter-end="opacity-100 scale-100"
                                 x-transition:leave="transition ease-in duration-75"
                                 x-transition:leave-start="opacity-100 scale-100"
                                 x-transition:leave-end="opacity-0 scale-95"
                                 class="absolute right-0 mt-4 w-[85vw] max-w-sm rounded-3xl bg-white dark:bg-zinc-900 shadow-2xl border border-gray-100 dark:border-zinc-800 py-3 z-50 overflow-hidden">
                                ...
                                <div class="max-h-72 overflow-y-auto">
                                    ...
                                    <template x-for="notif in notifications" :key="notif.id">
                                        <a :href="notif.data.url || '#'" @click="if(!notif.read_at) markAsRead(notif.id)" class="block px-4 py-3 hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors border-b border-gray-50 dark:border-zinc-800/50 last:border-0" :class="!notif.read_at ? 'bg-brand-50/50 dark:bg-brand-950/20' : ''">
                                            <div class="flex items-start gap-2">
                                                <div class="mt-1 flex-shrink-0">
                                                    <div class="w-1.5 h-1.5 rounded-full bg-brand-500" x-show="!notif.read_at"></div>
                                                </div>
                                                <div>
                                                    <p class="text-xs font-black text-gray-900 dark:text-white leading-tight" x-text="notif.data.title"></p>
                                                    <p class="text-[10px] text-gray-500 mt-1 leading-relaxed" x-text="notif.data.message"></p>
                                                    <p class="text-[9px] text-gray-400 mt-1 uppercase font-semibold" x-text="notif.created_at"></p>
                                                </div>
                                            </div>
                                        </a>
                ...
                <button onclick="toggleTheme()" class="p-2 text-slate-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
                    <svg class="w-5 h-5 hidden dark:block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v2.25m0 13.5V21m8.966-8.966h-2.25M3.75 12h2.25m13.364-7.364l-1.591 1.591M6.756 17.244l-1.591 1.591m12.728 0l-1.591-1.591M6.756 6.756L5.165 5.165M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" /></svg>
                    <svg class="w-5 h-5 block dark:hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" /></svg>
                </button>
                
                <button @click="isOpen = !isOpen" class="p-2 text-slate-600 dark:text-zinc-400 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
    ...
    <div x-show="isOpen" 
        ...
        <div class="px-4 pt-4 pb-6 space-y-1">
            ...
            <div class="pt-4 flex flex-col gap-2">
                 @auth
                    <div class="flex items-center gap-4 px-3 py-4 bg-gray-50 dark:bg-zinc-800/50 rounded-2xl mb-4 border border-gray-100 dark:border-zinc-800">
                        @if(Auth::user()->profile_photo_path)
                            <img src="{{ asset('storage/' . Auth::user()->profile_photo_path) }}" 
                                 class="h-12 w-12 rounded-full object-cover border-2 border-brand-500 shadow-sm" 
                                 alt="{{ Auth::user()->name }}">
                        @else
                            <div class="h-12 w-12 rounded-full bg-brand-900 flex items-center justify-center border-2 border-brand-500 shadow-sm">
                                <span class="text-white font-bold text-lg">{{ substr(Auth::user()->name, 0, 1) }}</span>
                            </div>
                        @endif
                        <div>
                            <p class="text-[10px] text-gray-500 uppercase tracking-widest font-black">Halo,</p>
                            <p class="text-base font-black text-gray-900 dark:text-white leading-tight">{{ Auth::user()->name }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1">
                        @if(Auth::user()->hasAnyRole(['Admin', 'Tim Produksi', 'Management/Owner']))
                            ...
                            @endphp
                            <a href="{{ $dashboardRoute }}" class="px-3 py-4 text-sm font-black text-brand-900 dark:text-brand-400 bg-brand-50 dark:bg-brand-950/20 rounded-xl flex items-center gap-3">
                        ...
                        @if(!Auth::user()->hasAnyRole(['Admin', 'Tim Produksi', 'Management/Owner']))
                            <a href="{{ route('customer.cart.index') }}" class="flex items-center justify-between px-3 py-3 text-sm font-bold rounded-xl hover:bg-gray-100 dark:hover:bg-zinc-800 group">
                            ...
                            <a href="{{ route('customer.orders') }}" class="flex items-center justify-between px-3 py-3 text-sm font-bold rounded-xl hover:bg-gray-100 dark:hover:bg-zinc-800 group">
                                <div class="flex items-center gap-3 text-gray-700 dark:text-gray-300">
                                    <svg class="w-5 h-5 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                            ...
                            <div class="h-px bg-gray-100 dark:border-zinc-800 my-2 mx-3"></div>
                            <a href="{{ route('customer.designs') }}" class="px-3 py-3 text-sm font-bold rounded-xl hover:bg-gray-100 dark:hover:bg-zinc-800 flex items-center gap-3 text-gray-700 dark:text-gray-300">
                                <svg class="w-5 h-5 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                Desain Saya
                            </a>
                        @endif

                        <a href="{{ route('profile.edit') }}" class="px-3 py-3 text-sm font-bold rounded-xl hover:bg-gray-100 dark:hover:bg-zinc-800 flex items-center gap-3 text-gray-700 dark:text-gray-300">
                            <svg class="w-5 h-5 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                            Pengaturan
                        </a>
                        
                        <div class="pt-4 border-t border-gray-100 dark:border-zinc-800 mt-2">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="w-full flex items-center justify-center gap-3 py-3 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 rounded-xl font-bold transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
                                    Log Out
                                </button>
                            </form>
