            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden relative">
                    ...
                    <div class="p-6 md:p-8">
                        ...
                        <div x-show="isLoadingShipping" class="mt-4 p-4 rounded-2xl bg-slate-50 border border-slate-200 shadow-sm flex items-center justify-center gap-3">
                            <svg class="animate-spin h-5 w-5 text-brand-900" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                            <span class="text-[10px] font-black text-slate-500 uppercase tracking-widest">Mencari Ekspedisi Terbaik...</span>
                        </div>
                            ...
                            <div class="flex items-center justify-between gap-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs font-bold text-emerald-700 mt-0.5" x-text="'Ongkir: Rp ' + formatRupiah(shippingCost) + (autoSelectedCourier && autoSelectedCourier.etd ? (' (Estimasi ' + autoSelectedCourier.etd.replace(/hari|days|day/gi, '').trim() + ' Hari)') : '')"></p>
                                    </div>
                                </div>
                    ...
                    <div class="divide-y divide-slate-50">
                        <template x-for="(item, idx) in items" :key="item.id">
                            <div class="p-6 md:p-8 space-y-8">
                                ...
                                <div class="pt-6 border-t border-slate-50 space-y-4">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="flex gap-2 p-1 bg-slate-100 rounded-xl">
                                            <button type="button" @click="item.designMethod = 'upload'" class="px-4 py-1.5 text-[9px] font-black uppercase rounded-lg transition-all" :class="item.designMethod === 'upload' ? 'bg-white shadow-sm text-brand-900' : 'text-slate-400'">Upload Gambar</button>
                                            <button type="button" @click="item.designMethod = 'customizer'" class="px-4 py-1.5 text-[9px] font-black uppercase rounded-lg transition-all" :class="item.designMethod === 'customizer' ? 'bg-white shadow-sm text-brand-900' : 'text-slate-400'">Dari Customizer</button>
                                        </div>
                                    </div>
                                ...
                                <div x-show="['jersey', 'jacket', 'setelan'].includes(item.category)">
                                ...
                                <div x-show="['jersey', 'jaket', 'setelan'].includes(item.category)">
                                                        ...
                                                        <template x-for="(player, pIdx) in item.roster" :key="pIdx">
                                                            <tr class="hover:bg-slate-50/50 transition-colors">
                                                                ...
                                                                <td class="px-2 py-3 relative">
                                                                    ...
                                                                    <div x-show="player.showUpgrades" @click.away="player.showUpgrades = false" class="absolute z-10 w-48 mt-1 right-2 bg-white border border-slate-200 rounded-xl shadow-xl p-2" style="display: none;">
                                                                        <div class="space-y-1 max-h-40 overflow-y-auto custom-scrollbar">
                                                                            @foreach($upgrades as $cat => $list)
                                                                                <div class="px-2 py-1 bg-slate-50 text-[8px] font-black text-slate-400 uppercase tracking-widest rounded">{{ $cat }}</div>
                                                                                @foreach($list as $upgrade)
                                                                                    <label class="flex items-center gap-2 px-2 py-1.5 hover:bg-slate-50 rounded cursor-pointer">
                                                                                        <input type="checkbox" x-model="player.upgrades" value="{{ $upgrade->id }}" :name="'roster['+item.id+']['+pIdx+'][upgrades][]'" class="w-3.5 h-3.5 text-brand-900 border-slate-300 rounded focus:ring-brand-900">
                                                                                        <div class="flex-1 min-w-0">
                                                                                            <p class="text-[9px] font-bold text-slate-700 truncate">{{ $upgrade->name }}</p>
                                                                                            <p class="text-[8px] text-amber-600 font-black leading-none">+{{ number_format($upgrade->price/1000, 0) }}k</p>
                                                                                        </div>
                                                                                    </label>
                                                                                @endforeach
                                                                            @endforeach
                                ...
                                <div x-show="!['jersey', 'jacket', 'setelan'].includes(item.category)" class="pt-6 border-t border-slate-50 relative">
                                    <h4 class="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-3">2. Pilihan Ukuran</h4>
                                    <div class="flex items-center justify-between mb-3">
                                        <p class="text-[9px] text-slate-500 font-medium">Tentukan jumlah ukuran sesuai total pesanan (<span class="font-bold text-slate-800" x-text="item.qty"></span> pcs).</p>
                                        <span class="text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md" 
                                              :class="(item.qty - Object.values(item.sizeQuantities).reduce((a, b) => a + (parseInt(b) || 0), 0)) === 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'"
                                              x-text="'Sisa Kuota: ' + (item.qty - Object.values(item.sizeQuantities).reduce((a, b) => a + (parseInt(b) || 0), 0)) + ' pcs'"></span>
                                    </div>
                                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                                        <template x-for="sz in ['S', 'M', 'L', 'XL', 'XXL', '3XL', '4XL']">
                                            <div class="flex items-center gap-2 border border-slate-200 rounded-lg px-3 py-2 bg-slate-50 hover:border-brand-900 transition-colors">
                                                <span class="text-[10px] font-black w-6 text-slate-700" x-text="sz"></span>
                                                <input type="number" min="0" 
                                                       :max="item.qty - (Object.keys(item.sizeQuantities).reduce((acc, key) => acc + (key !== sz ? (parseInt(item.sizeQuantities[key]) || 0) : 0), 0))"
                                                       :disabled="(parseInt(item.sizeQuantities[sz]) || 0) === 0 && (Object.values(item.sizeQuantities).reduce((a, b) => a + (parseInt(b) || 0), 0) >= item.qty)"
                                                       x-model.number="item.sizeQuantities[sz]" 
                                                       :name="'size_quantities['+item.id+']['+sz+']'" 
                                                       class="w-full text-xs font-bold px-2 py-1 border border-slate-200 rounded text-center focus:border-brand-500 focus:outline-none disabled:bg-slate-100 disabled:opacity-50" placeholder="0">
                                            </div>
                                        </template>
                                    </div>
                                </div>
                        ...
                        <template x-for="(opt, idx) in allShippingOptions" :key="idx">
                            <div @click="applyShippingOption(opt, false); showCourierOptionsModal = false"
                                ...
                                <div class="space-y-1">
                                    ...
                                    <p class="text-[10px] text-slate-400 font-bold" x-text="'Estimasi: ' + (opt.etd ? opt.etd.replace(/hari|days|day/gi, '').trim() + ' Hari' : '-')"></p>
                                </div>
        ...
        return {
            ...
            items: [
                ...
                @endphp
                {
                    ...
                    category: '{{ addslashes(strtolower($item->package->category ?? "")) }}',
                    ...
                    sizeQuantities: { S: 0, M: 0, L: {{ $item->quantity }}, XL: 0, XXL: 0, '3XL': 0, '4XL': 0 },
