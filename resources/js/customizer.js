export default () => ({
    ...
    get activeState() {
        return this.viewStates[this.currentView];
    },
    get partColors() {
        return this.activeState.partColors;
    },
    get partColorMode() {
        return this.activeState.partColorMode || {};
    },
    get partActiveGradientPreset() {
        return this.activeState.partActiveGradientPreset || {};
    },
    get partPatterns() {
        return this.activeState.partPatterns;
    },
    get partPatternScales() {
        return this.activeState.partPatternScales;
    },
    get partPatternAngles() {
        return this.activeState.partPatternAngles;
    },
    get partPatternFlipX() {
        return this.activeState.partPatternFlipX;
    },
    get partPatternFlipY() {
        return this.activeState.partPatternFlipY;
    },
    get partActivePatterns() {
        return this.activeState.partActivePatterns;
    },
    get partPatternColors() {
        return this.activeState.partPatternColors;
    },
    get partPatternX() {
        return this.activeState.partPatternX;
    },
    get partPatternY() {
        return this.activeState.partPatternY;
    },
    get patternColor() {
        return this.partPatternColors[this.activePatternPart] || "#000000";
    },
    get partPatternGradientEnabled() {
        return this.activeState.partPatternGradientEnabled || {};
    },
    get partPatternGradientColor1() {
        return this.activeState.partPatternGradientColor1 || {};
    },
    get partPatternGradientColor2() {
        return this.activeState.partPatternGradientColor2 || {};
    },
    get partPatternGradientAngle() {
        return this.activeState.partPatternGradientAngle || {};
    },
    get partPatternGradientType() {
        return this.activeState.partPatternGradientType || {};
    },
    ...
    availableFonts: [
        { id: "sans-serif", name: "Standard", family: "sans-serif" },
        {
            id: "AC Milan 4th",
            name: "AC Milan 4th",
            family: "'AC Milan 4th', sans-serif",
        },
        {
            id: "Brondby IF",
            name: "Brondby IF",
            family: "'Brondby IF', sans-serif",
        },
        {
            id: "Girondins Bordeaux",
            name: "Girondins Bordeaux",
            family: "'Girondins Bordeaux', sans-serif",
        },
        {
            id: "Iraq 2025",
            name: "Iraq 2025",
            family: "'Iraq 2025', sans-serif",
        },
        {
            id: "Osasuna 25-26",
            name: "Osasuna 25-26",
            family: "'Osasuna 25-26', sans-serif",
        },
        {
            id: "Palermo FC",
            name: "Palermo FC",
            family: "'Palermo FC', sans-serif",
        },
        {
            id: "Portugal WC 2026",
            name: "Portugal WC 2026",
            family: "'Portugal WC 2026', sans-serif",
        },
        {
            id: "PSG Fourth",
            name: "PSG Fourth",
            family: "'PSG Fourth', sans-serif",
        },
        {
            id: "SC Freiburg",
            name: "SC Freiburg",
            family: "'SC Freiburg', sans-serif",
        },
        {
            id: "South Africa",
            name: "South Africa",
            family: "'South Africa', sans-serif",
        },
        {
            id: "Spain WC 2026",
            name: "Spain WC 2026",
            family: "'Spain WC 2026', sans-serif",
        },
    ],
    ...
    get activeMenuLabel() {
        return this.menus.find((m) => m.id === this.activeMenu)?.label || "";
    },
            ...
            this.initCanvas()
                .then(async () => {
                    ...
                    this.updateDesignUrl =
                        document.getElementById("update-design-url")?.value ||
                        "";

                    // Load existing design if available
                    const existingData = document.getElementById(
                        "existing-design-data",
                    )?.value;
                    const existingName = document.getElementById(
                        "existing-design-name",
                    )?.value;
                    if (existingData) {
                        try {
    ...
    async initCanvas() {
        try {
            ...
            // Custom styles for selected objects to be highly visible
            window.fabric.Object.prototype.set({
                transparentCorners: false,
                borderColor: "#10b981", // green border
                cornerColor: "#ffffff",
                cornerStrokeColor: "#10b981",
                cornerSize: 14, // Larger corners
                padding: 12,
                cornerStyle: "circle",
                borderDashArray: [5, 5],
            });
    ...
    async setView(view) {
        ...
        const mockup = this.availableMockups.find(
            (m) => m.id === this.currentModel,
        );
        const viewParts = mockup?.parts[view] || [];

        if (view === "pants") {
            this.activePart = viewParts.includes("left")
                ? "left"
                : viewParts[0];
        } else {
            this.activePart = viewParts.includes("body")
                ? "body"
                : viewParts[0];
        }
    ...
    async ensureAllSnapshots() {
        ...
        const viewsToRender = ["front", "back", "pants"].filter(
            (v) => mockup.parts[v] && mockup.parts[v].length > 0,
        );
        const viewMap = {
            front: "jersey_front",
            back: "jersey_back",
    ...
    async exportDesignHD(format) {
        const mult = 4; // HD (2400x2400)
        const canvas = await this.generateCompositeCanvas(
            600 * mult,
            600 * mult,
            format,
        );
    ...
    async loadInitialLayers() {
        ...
        this.layers = {
            shadows: null,
            highlights: null,
            partPatterns: this.layers.partPatterns || {},
        };
        ...
        try {
            ...
            const potentialPartIds =
                mockup?.parts[this.currentView] ||
                (this.currentView === "pants"
                    ? ["left", "right", "belt", "list1", "list2"]
                    : [
                          "body",
            ...
            for (const pid of potentialPartIds) {
                ...
                const layer = await this.loadImage(src, {
                    isSystemLayer: true,
                    selectable: false,
                    evented: false,
                });
            ...
            this.layers.shadows = await this.loadImage(shadowSrc, {
                isSystemLayer: true,
                selectable: false,
                evented: false,
                globalCompositeOperation: "multiply",
            });
            this.layers.highlights = await this.loadImage(highlightSrc, {
                isSystemLayer: true,
                selectable: false,
                evented: false,
            ...
            for (const pid of Object.keys(
                this.activeState.partGradientEnabled || {},
            )) {
                if (
                    this.activeState.partGradientEnabled[pid] &&
                    this.layers[pid]
                ) {
    ...
    async applyFilterToLayer(partId, color) {
        ...
        layer.filters = [
            new window.fabric.filters.BlendColor({
                color: color,
                mode: "multiply",
                alpha: 1,
            }),
    ...
    async _applySolidToSingle(partId, color) {
        ...
        if (this.activeState.partColorMode)
            this.activeState.partColorMode[partId] = "solid";
        if (this.activeState.partGradientEnabled)
            this.activeState.partGradientEnabled[partId] = false;
        if (this.activeState.partActiveGradientPreset)
            this.activeState.partActiveGradientPreset[partId] = null;
        if (this.layers.partGradients && this.layers.partGradients[partId])
    ...
    async _applyGradientPresetToSingle(partId, preset) {
        ...
        if (this.activeState.partColorMode)
            this.activeState.partColorMode[partId] = "gradient";
    ...
    async updatePartGradient(partId, key, val) {
        if (!this.activeState.partGradientEnabled.hasOwnProperty(partId))
            return;
        if (key) {
            this.activeState[key][partId] = val;
        } else {
            this.activeState.partGradientEnabled[partId] =
                !this.activeState.partGradientEnabled[partId];
        }
    ...
    async updatePatternGradient(key, val) {
        ...
        if (key) {
            ...
            // Toggle enabled
            this.activeState.partPatternGradientEnabled[pid] =
                !this.activeState.partPatternGradientEnabled[pid];
        }
    ...
    async applyPatternFilters(pid, pLayer) {
        ...
        if (isGradient) {
            ...
            filters.push(
                new window.fabric.filters.BlendImage({
                    image: gradImg,
                    mode: "multiply",
                    alpha: 1,
                }),
    ...
    async updatePattern() {
        ...
        for (const pid of Object.keys(this.partPatterns)) {
            ...
            const pLayer = await this.loadImage(patternObj.url, {
                ...
                isSystemLayer: true,
                clipPath: maskGroup,
                scaleX: this.partPatternScales[pid] || 1,
                scaleY: this.partPatternScales[pid] || 1,
                angle: this.partPatternAngles[pid] || 0,
                flipX: this.partPatternFlipX[pid] || false,
                flipY: this.partPatternFlipY[pid] || false,
                left: this.activeState.partPatternX[pid] ?? 300,
                top: this.activeState.partPatternY[pid] ?? 300,
            });
    ...
    updatePatternFlip(axis) {
        const pid = this.activePatternPart;
        if (axis === "X")
            this.activeState.partPatternFlipX[pid] =
                !this.activeState.partPatternFlipX[pid];
        else
            this.activeState.partPatternFlipY[pid] =
                !this.activeState.partPatternFlipY[pid];
        this.updatePattern();
    },
    ...
    updatePatternPosition(axis, val) {
        ...
        if (this.layers.partPatterns && this.layers.partPatterns[pid]) {
            this.layers.partPatterns[pid].set(
                axis === "X" ? "left" : "top",
                parseFloat(val),
            );
            this.canvas.requestRenderAll();
    ...
    resetPatternProperty(prop) {
        ...
        const pid = this.activePatternPart;
        if (prop === "scale") {
            this.activeState.partPatternScales[pid] = 0.3;
            this.updatePatternScale(0.3);
        } else if (prop === "angle") {
            this.activeState.partPatternAngles[pid] = 0;
            this.updatePatternAngle(0);
        } else if (prop === "X") {
            this.activeState.partPatternX[pid] = 300;
            this.updatePatternPosition("X", 300);
        } else if (prop === "Y") {
            this.activeState.partPatternY[pid] = 300;
            this.updatePatternPosition("Y", 300);
        }
        this.saveHistory();
    },

    togglePartPattern(partId) {
        this.activeState.partPatterns[partId] =
            !this.activeState.partPatterns[partId];
        if (
            this.activeState.partPatterns[partId] &&
            !this.activeState.partActivePatterns[partId]
        ) {
    ...
    async getMockupClipMask() {
        ...
        for (const pid of partIds) {
            if (this.layers[pid])
                maskLayers.push(
                    await this.layers[pid].clone(["isSystemLayer"]),
                );
        }
        return maskLayers.length
            ? new window.fabric.Group(maskLayers, {
                  absolutePositioned: true,
                  originX: "center",
                  originY: "center",
                  left: 300,
    ...
    renderLayers() {
        ...
        const trimParts =
            this.currentView === "pants"
                ? ["belt", "list1", "list2"]
                : ["collar", "collar1", "left_cuff", "right_cuff"];
        ...
        const renderGroup = (partsArray) => {
            ...
            if (this.layers.partGradients) {
                partsArray.forEach((key) => {
                    ...
                    if (g && this.activeState.partGradientEnabled[key])
                        this.canvas.add(g);
                });
            }
        ...
        this.designObjects
            .filter((obj) => obj.view === this.currentView)
            .forEach((obj) => this.canvas.add(obj));
    ...
    async updateTextProperty(key, val) {
        ...
        if (
            ...
            if (key === "activeFont") {
                try {
                    await document.fonts.load(`10px "${val}"`);
                } catch (e) {}
            ...
            if (key === "textFontSize")
                activeObj.set({ fontSize: parseFloat(val) });
            if (key === "textCharSpacing")
                activeObj.set({ charSpacing: parseFloat(val) });
            ...
            if (
                ...
                if (this.textGradientEnabled) {
                    ...
                    const gradient = new window.fabric.Gradient({
                        ...
                        coords: coords,
                        colorStops: [
                            { offset: 0, color: this.textGradientColor1 },
                            { offset: 1, color: this.textGradientColor2 },
                        ],
                    });
    ...
    async addText() {
        ...
        const newText = new window.fabric.IText(this.textInput || "TEKS BARU", {
            ...
            charSpacing: this.textCharSpacing,
            cornerStyle: "circle",
            cornerColor: "#06402B",
            clipPath: clipMask,
    ...
    generateGradientImage(width, height, color1, color2, angle, type) {
        ...
        if (type === "linear") {
            ...
            grd = ctx.createLinearGradient(
                cx - Math.cos(rad) * halfLen,
                cy - Math.sin(rad) * halfLen,
                cx + Math.cos(rad) * halfLen,
                cy + Math.sin(rad) * halfLen,
            );
        } else {
        ...
        this.showCopyConfirmModal = false;
        {
            ...
            for (let targetKey in toState.partColors) {
                ...
                if (!fromState.partColors.hasOwnProperty(srcKey)) {
                    ...
                    else if (["belt", "list1", "list2"].includes(targetKey))
                        srcKey = "collar";
                    else continue;
                }

                toState.partColors[targetKey] = fromState.partColors[srcKey];
                toState.partColorMode[targetKey] =
                    fromState.partColorMode[srcKey];
                toState.partActiveGradientPreset[targetKey] =
                    fromState.partActiveGradientPreset[srcKey];
                toState.partPatterns[targetKey] =
                    fromState.partPatterns[srcKey];
                toState.partPatternScales[targetKey] =
                    fromState.partPatternScales[srcKey];
                toState.partPatternAngles[targetKey] =
                    fromState.partPatternAngles[srcKey];
                ...
                if (
                    ...
                    toState.partPatternX[targetKey] =
                        600 - fromState.partPatternX[srcKey];
                    // Cerminkan juga arah FlipX agar coraknya presisi sama seperti dari depan
                    toState.partPatternFlipX[targetKey] =
                        !fromState.partPatternFlipX[srcKey];
                } else {
                    toState.partPatternX[targetKey] =
                        fromState.partPatternX[srcKey];
                    toState.partPatternFlipX[targetKey] =
                        fromState.partPatternFlipX[srcKey];
                }
                // ----------------------------------------------------

                toState.partPatternFlipY[targetKey] =
                    fromState.partPatternFlipY[srcKey];
                toState.partActivePatterns[targetKey] =
                    fromState.partActivePatterns[srcKey];
                toState.partPatternColors[targetKey] =
                    fromState.partPatternColors[srcKey];
                toState.partPatternY[targetKey] =
                    fromState.partPatternY[srcKey];
                toState.partGradientEnabled[targetKey] =
                    fromState.partGradientEnabled[srcKey];
                toState.partGradientColor1[targetKey] =
                    fromState.partGradientColor1[srcKey];
                toState.partGradientColor2[targetKey] =
                    fromState.partGradientColor2[srcKey];
                toState.partGradientAngle[targetKey] =
                    fromState.partGradientAngle[srcKey];
                toState.partGradientType[targetKey] =
                    fromState.partGradientType[srcKey];
                toState.partPatternGradientEnabled[targetKey] =
                    fromState.partPatternGradientEnabled[srcKey];
                toState.partPatternGradientColor1[targetKey] =
                    fromState.partPatternGradientColor1[srcKey];
                toState.partPatternGradientColor2[targetKey] =
                    fromState.partPatternGradientColor2[srcKey];
                toState.partPatternGradientAngle[targetKey] =
                    fromState.partPatternGradientAngle[srcKey];
                toState.partPatternGradientType[targetKey] =
                    fromState.partPatternGradientType[srcKey];
            }
    ...
    updateToolbar(e) {
        ...
        this.toolbarPos = {
            top: rect.top * vpt[0] + vpt[5] - 110,
            left: rect.left * vpt[0] + vpt[4] + (rect.width * vpt[0]) / 2,
        };
    ...
    setupZoomEvents() {
        ...
        // Panning with Alt + Drag (Desktop) or Middle Mouse
            ...
            container.addEventListener(
                ...
                (e) => {
                    if (e.touches.length === 2 && initialDistance) {
                        ...
                        let zoom = Math.min(
                            Math.max(initialZoom * scale, 0.5),
                            5,
                        );
                        let centerX =
                        ...
                        if (initialPan) {
                            ...
                            vpt[4] += centerX - initialPan.x;
                            vpt[5] += centerY - initialPan.y;
                            this.canvas.requestRenderAll();
                        }
                        initialPan = { x: centerX, y: centerY };
                        e.preventDefault();
                    }
                },
                { passive: false },
    ...
    async loadImage(url, options = {}) {
        try {
            const img = await window.fabric.FabricImage.fromURL(url, {
    ...
    triggerLogoUpload() {
        ...
        input.onchange = async (e) => {
            ...
            reader.onload = async (ev) => {
                const img = await window.fabric.FabricImage.fromURL(
                    ev.target.result,
                );
    ...
    openCropper(imageSrc) {
        ...
        if (
            !src &&
            activeObj &&
            (activeObj.type === "image" || activeObj._element)
        ) {
            // Gunakan gambar aslinya, bukan gambar canvas yang sudah di skala/rotasi
            src = activeObj._element
                ? activeObj._element.src
                : activeObj.getSrc();
        }
    ...
    async applyCrop() {
        ...
        if (
            ...
            const localCx =
                cropData.x + cropData.width / 2 - activeObj.width / 2;
            const localCy =
                cropData.y + cropData.height / 2 - activeObj.height / 2;
            ...
            this.designObjects = this.designObjects.filter(
                (o) => o !== activeObj,
            );
            this.designObjects.push(img);
            this.canvas.add(img);
    ...
    setActiveMenu(menuId) {
        if (this.activeMenu === menuId) this.togglePanel();
        else {
    ...
    deleteSelected() {
        ...
        if (obj) {
            ...
            this.designObjects = this.designObjects.filter((o) => o !== obj);
            this.showToolbar = false;
        }
    },
    resizeCanvas() {
        ...
        try {
            ...
            if (result.success) {
                ...
                const targetUrl =
                    result.redirect +
                    (this.packageSlug
                        ? "?package=" + encodeURIComponent(this.packageSlug)
                        : "");
